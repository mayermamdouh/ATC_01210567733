...
export default function UpdateEvent() {
      ...
      const fetchEvent = async () => {
        try {
          ...
          if (response.data.status === "success") {
            ...
            const { _id, __v, ...eventData } = response.data.data.event;
            ...
            setPreview(
              `${import.meta.env.VITE_API_URL}/uploads/${eventData.photo}`
            );
            setFormData(eventData);
            setOldFormData(eventData);
          }
        } catch (e) {
  ...
  useEffect(() => {
    if (oldFormData) {
      setIsChanged(JSON.stringify(formData) === JSON.stringify(oldFormData));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.status === "success") {
        ...
        setMessage("You have successfully update the event!");
      }
    } catch (e) {
      setMessage(`${e.message}`);
    }finally{
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col border-2 rounded-md w-3/4 mx-auto bgEventAdmin border-gray-200 p-4">
          ...
          <div className="mt-7 flex gap-3 flex-col">
            <div className="flex flex-row items-center gap-5">
              <div className="flex flex-col items-start justify-center gap-2 w-[45%]">
                ...
                <select
                  id="Category"
                  className="bg-gray-50 text-black border-[2px] border-gray-300  text-sm rounded-lg outline-none block w-full p-2.5 cursor-pointer focus:border-[#124e66]"
              ...
              <div className=" w-[45%] flex flex-col gap-2">
                ...
                <input
                  ...
                  value={formData.date}
                  type="datetime-local"
                  className="bg-gray-50 border-[2px] text-black focus:border-[#124e66] border-gray-300 text-sm rounded-lg  block w-full  p-2.5 cursor-pointer select-none outline-none"
                ></input>
              </div>
            </div>
            <InputField
      ...
      <div className="flex flex-row items-center justify-center mb-10">
        <button type="submit" className="my-5 w-1/3" disabled={isChanged}>
          {t("update_event")}
        </button>
      </div>
    </form>
